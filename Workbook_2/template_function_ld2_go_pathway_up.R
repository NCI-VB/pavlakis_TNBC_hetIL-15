...
ld2_go_pathway_up <- function(DEG_lymph,Ortholog_Map_for_RNA_Seq) {
    # image: png
    ...
    DEG_lymph %>% dplyr::select("Gene", "IL15_day_2-control_day_2_tstat") %>% dplyr::collect() -> genesmat
    genesmat %>% dplyr::arrange(desc(`IL15_day_2-control_day_2_tstat`)) -> genesmat
...
print("template_function_ld2_go_pathway_up.R #########################################################################")
...
var_ld2_go_pathway_up<-ld2_go_pathway_up(var_DEG_lymph,var_Ortholog_Map_for_RNA_Seq)
invisible(graphics.off())
saveRDS(var_ld2_go_pathway_up, paste0(rds_output,"/var_ld2_go_pathway_up.rds"))
