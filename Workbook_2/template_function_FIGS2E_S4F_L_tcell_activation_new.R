FIGS2E_S4F_L_tcell_activation_new <- function(ld2_go_collect,DEG_lymph, metadata_fixed, Ortholog_Map_for_RNA_Seq) {
    # image: png
    ...
    table_to_convert = ld2_go_collect

    gene_column_to_convert = "T_cell_activation_involved_in_immune_response_GO_0002286"
    organism = "Mouse"
    if(organism != "Human"){
...
dplyr::rename("orthology_conversion" = orthology_conversion_column ) %>% dplyr::select("orthology_reference", "orthology_conversion") -> orthology_table
    ...
    table_to_convert %>%
        ...
        genes = table_to_convert %>% dplyr::select("T_cell_activation_involved_in_immune_response_GO_0002286")
    ...
    pathname <- stringr::str_replace_all("T-cell_Activation", "_", " ") 
   pathname <- stringr::str_wrap(pathname,30)
    ...
    if (FALSE == FALSE) {
        ...
        genes_to_include_df <- dplyr::withColumnRenamed(genes_to_include_df, "T_cell_activation_involved_in_immune_response_GO_0002286", "gene_name_column_temp")
        spark_df %>% 
    ...
    png(
      filename="FIGS2E_S4F_L_tcell_activation_new.png",
      width=14,
      height=10,
      units="in",
      pointsize=4,
...
print("template_function_FIGS2E_S4F_L_tcell_activation_new.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_ld2_go_collect<-readRDS(paste0(rds_output,"/var_ld2_go_collect.rds"))
Input_is_Seurat_count <- 0
for(item in var_ld2_go_collect){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_ld2_go_collect<-as.data.frame(var_ld2_go_collect)}else{var_ld2_go_collect <- var_ld2_go_collect}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_FIGS2E_S4F_L_tcell_activation_new<-FIGS2E_S4F_L_tcell_activation_new(var_ld2_go_collect,var_DEG_lymph,var_metadata_fixed,var_Ortholog_Map_for_RNA_Seq)
invisible(graphics.off())
saveRDS(var_FIGS2E_S4F_L_tcell_activation_new, paste0(rds_output,"/var_FIGS2E_S4F_L_tcell_activation_new.rds"))
