...
ld2_go_collect <- function(ld2_go_pathway_up) {
    ## Set-up.
    suppressMessages(library(dplyr))
    df <- dplyr::collect(ld2_go_pathway_up)
...
print("template_function_ld2_go_collect.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_ld2_go_pathway_up<-readRDS(paste0(rds_output,"/var_ld2_go_pathway_up.rds"))
Input_is_Seurat_count <- 0
for(item in var_ld2_go_pathway_up){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_ld2_go_pathway_up<-as.data.frame(var_ld2_go_pathway_up)}else{var_ld2_go_pathway_up <- var_ld2_go_pathway_up}
invisible(graphics.off())
var_ld2_go_collect<-ld2_go_collect(var_ld2_go_pathway_up)
invisible(graphics.off())
saveRDS(var_ld2_go_collect, paste0(rds_output,"/var_ld2_go_collect.rds"))
