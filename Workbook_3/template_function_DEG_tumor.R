...
DEG_tumor <- function(tumor_filtered, metadata_fixed) {
    suppressMessages(library(limma))
    suppressMessages(library(tidyverse))
    suppressMessages(library(edgeR))
    suppressMessages(library(stringr))
    df <- dplyr::collect(tumor_filtered)

    samples_for_deg_analysis = c("Tumor_CTRL_321","Tumor_CTRL_327","Tumor_CTRL_331","Tumor_IL15_320","Tumor_IL15_338","Tumor_IL15_339","Tumor_CTRL_203","Tumor_CTRL_205","Tumor_CTRL_206","Tumor_IL15_209","Tumor_IL15_215","Tumor_IL15_216","Tumor_Control_301","Tumor_Control_324","Tumor_IL15_Treated_684","Tumor_IL15_Treated_336","Tumor_IL15_Treated_340")
...
print("template_function_DEG_tumor.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
var_tumor_filtered<-readRDS(paste0(rds_output,"/var_tumor_filtered.rds"))
Input_is_Seurat_count <- 0
for(item in var_tumor_filtered){ if (class(item)=="Seurat"){Input_is_Seurat_count = Input_is_Seurat_count + 1}}
if(Input_is_Seurat_count == 0 ){
var_tumor_filtered<-as.data.frame(var_tumor_filtered)}else{var_tumor_filtered <- var_tumor_filtered}
currentdir <- getwd()
rds_output <- paste0(currentdir,'/rds_output')
...
var_DEG_tumor<-DEG_tumor(var_tumor_filtered,var_metadata_fixed)
invisible(graphics.off())
saveRDS(var_DEG_tumor, paste0(rds_output,"/var_DEG_tumor.rds"))
