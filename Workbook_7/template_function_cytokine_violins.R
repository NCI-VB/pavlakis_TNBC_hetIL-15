cytokine_violins <- function(Labeled_so, Get_Metadata, unnamed, Gene_list) {
    ...
    g <- ggplot(df.melt, aes_string(x=ident_of_interest, y=expression_data, fill=ident_of_interest)) +
        ...
        labs(y=axis.title.y) +
        theme(  strip.text.x = element_text( color="black", face="plain", size=32),
...
print("template_function_cytokine_violins.R #########################################################################")
...
var_cytokine_violins<-cytokine_violins(var_Labeled_so,var_Get_Metadata,var_unnamed,var_Gene_list)
invisible(graphics.off())
saveRDS(var_cytokine_violins, paste0(rds_output,"/var_cytokine_violins.rds"))
